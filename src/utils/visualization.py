...
class MetricsPrint():
    ...
    -----------
      ...
      - metric_names : list of names of the metrics to display
      - n_samples_batch : number of samples in a batch

    Attributes:
    -----------
      - metrics_names : list of names of the metrics to display
      - n_samples_batch : number of samples in a batch
      - column_size : size of the columns

    Methods:
    --------
      - initial_print : function to print the header of the table
      - print_loss_metrics : function to print the metrics of a batch
    """
...
class TimePredictor():
    ...
    def lap(self):
        ...
        self.last = time.time()
        self.mean_iter = self.beta * elapsed + \
            (1 - self.beta) * self.mean_iter if self.mean_iter != None else elapsed
        return elapsed
