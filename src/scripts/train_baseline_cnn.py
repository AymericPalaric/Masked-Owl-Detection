...
if __name__ == "__main__":
  ...
  image_transform = torchvision.transforms.Compose([torchvision.transforms.ToTensor(), torchvision.transforms.Resize(reshape_size), torchvision.transforms.Normalize(mean=mean, std=std)])

  # Datasets
  train_dataset = torch_utils.create_classif_dataset(audio_transform, image_transform, train_test=True)
  test_dataset = torch_utils.create_classif_dataset(audio_transform, image_transform, train_test=False)

  # Dataloaders
  train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=n_workers, pin_memory=True if torch.cuda.is_available() else False)
  test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=n_workers, pin_memory=True if torch.cuda.is_available() else False)

  print(f"Training on {len(train_dataset)} samples, testing on {len(test_dataset)} samples")
  print(f"Training on {len(train_dataloader)} batches, testing on {len(test_dataloader)} batches")
