...
if __name__ == "__main__":
  ...
  #print("mAP: ", mAP)
  for file in files:
    audio, fs = audio_utils.load_audio_file(os.path.join("data/detection_samples/samples", f"{file}.wav"))
    lbl = np.load(os.path.join("data/detection_samples/targets", f"{file}.npy"))
    bbxs, scores = pipeline(audio)
    ts, f, spectro = audio_utils.compute_spectrogram(audio, fs, nperseg=256, noverlap=256//4, scale="dB")
    print(spectro.shape)
