...
def create_negative_samples(raw_folder: str, output_folder: str, negative_sample_duration: float, n_samples: int, uuid: int, log: bool = True) -> None:
    ...
    for file in tqdm(files, disable=not log):
        ...
        for i in np.random.permutation(n_splits)[:negative_samples_per_file]:
            audio_utils.save_audio_file(os.path.join(
                output_folder, f"{uuid}_{idx}.wav"), splitted_data[i], fs)
            idx += 1
...
def split_samples_test_train(positive_samples_folder: str, negative_samples_folder: str, hard_samples_folder: str, p_train: float, p_hard: float) -> None:
    ...
    positive_files = [file for file in os.listdir(
        positive_samples_folder) if file.endswith(".wav")]
    negative_files = [file for file in os.listdir(
        negative_samples_folder) if file.endswith(".wav")]
    hard_files = [file for file in os.listdir(
        hard_samples_folder) if file.endswith(".wav")]
    ...
    for file in train_positive_files:
        os.rename(os.path.join(positive_samples_folder, file),
                  os.path.join(train_positive_folder, file))
    for file in train_negative_files:
        os.rename(os.path.join(negative_samples_folder, file),
                  os.path.join(train_negative_folder, file))
    for file in train_hard_files:
        os.rename(os.path.join(hard_samples_folder, file),
                  os.path.join(train_hard_folder, file))
    for file in test_positive_files:
        os.rename(os.path.join(positive_samples_folder, file),
                  os.path.join(test_positive_folder, file))
    for file in test_negative_files:
        os.rename(os.path.join(negative_samples_folder, file),
                  os.path.join(test_negative_folder, file))
    for file in test_hard_files:
        os.rename(os.path.join(hard_samples_folder, file),
                  os.path.join(test_hard_folder, file))
...
def create_detection_dataset(output_folder: str, raw_folder: str, positive_folder: str, hard_folder: str, detection_duration: float, n_samples: int, uuid: int, call_proportion: float, hard_call_proportion: float, log: bool = True) -> None:
    ...
    # set seed
    np.random.seed(0)
    raw_files = [file for file in os.listdir(
        raw_folder) if file.endswith(".wav")]
    positive_files = [file for file in os.listdir(
        positive_folder) if file.endswith(".wav")]
    hard_files = [file for file in os.listdir(
        hard_folder) if file.endswith(".wav")]
        ...
        for j in range(len(splitted_data)):
            ...
            for k in indices_call:
                positive_file_path = os.path.join(
                    positive_folder, positive_files[k])
                positive_data, _ = audio_utils.load_audio_file(
                    positive_file_path, sr=fs)

                positive_data = extract_data(positive_data)
                add_call_raw_data(data_split, call_indices,
                                  positive_data, positif=True)
            ...
            for k in indices_call:
                ...
                hard_data, _ = audio_utils.load_audio_file(
                    hard_file_path, sr=fs)

                hard_data = extract_data(hard_data)
                add_call_raw_data(data_split, call_indices,
                                  hard_data, positif=False)

            call_indices = np.array(call_indices)
            #  save audio and positive call indices
            audio_utils.save_audio_file(os.path.join(
                output_folder, 'samples', f"{uuid}_{i}_{j}.wav"), data_split, fs)
            np.save(os.path.join(output_folder, 'target',
                    f"{uuid}_{i}_{j}.npy"), call_indices)
